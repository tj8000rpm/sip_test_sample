...
class OneShotCaptureThread(threading.Thread):
    output=None
    timeout=30
    ...
    def run(self):
        ...
        output=None
        try:
            caps = pyshark.LiveCapture(interface='11',bpf_filter='ip and udp')
            caps.sniff(timeout=self.timeout)
            output=[]
            for packet in caps._packets:
                ############################################################
                ### パケット受信時刻の取得
                ############################################################
...
class Test_Sample(unittest.TestCase):
    def test_sample1(self):
        timeout=5
        myth=OneShotCaptureThread(timeout)
        myth.start()
        #time.sleep(0.02)
